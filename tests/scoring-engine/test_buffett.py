"""Tests for skills.scoring_engine.personas.buffett (M3.7).
...
from skills.scoring_engine.personas import buffett
...
def test_happy_path_produces_expected_shape() -> None:
    ...
    expected_sub_scores = {
        "fundamentals", "consistency", "moat",
        "management_quality", "pricing_power", "book_value_growth",
    }
...
def test_determinism_byte_equal_across_two_runs() -> None:
    metrics = _load("infy.json")
    a = buffett.run(metrics)
    b = buffett.run(metrics)
...
def test_insufficient_data_path_exits_zero_and_reports_shortfall() -> None:
    metrics = _load("buffett_insufficient_data.json")
    result = buffett.run(metrics)
...
def test_regression_snapshot_sub_scores_and_weighted_score() -> None:
    ...
    Values computed once from infy.json against the port at M3.7 landing.
    """
    metrics = _load("infy.json")
    result = buffett.run(metrics)
    ...
    assert result["weighted_score"] == 74
